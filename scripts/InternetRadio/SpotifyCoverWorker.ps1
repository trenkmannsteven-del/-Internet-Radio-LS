...
$logPath = Join-Path $BaseDir "spotify_cover_worker.log"
...
function Draw-SpotifyArtworkCard([Drawing.Graphics]$gfx, [Drawing.Image]$img, [int]$side) {
    # TEST67: Spotify artwork remains in its original form. It is only scaled to fit,
    # centred, and clipped to the documented rounded corners. No crop, blur, tint,
    # overlay or duplicated artwork background is applied.
    $brandBg = [Drawing.Color]::FromArgb(255, 25, 20, 20)
    $gfx.Clear($brandBg)
    ...
    # 8px radius at 512px output: restrained optical blending on a large display.
    $radius = 8
    ...
    # Surrounding frame is outside the artwork and does not alter its pixels.
    $framePen = New-Object Drawing.Pen([Drawing.Color]::FromArgb(92, 255, 255, 255), 1.0)
...
# vom Haupt-Bridge-Prozess hart beendet werden, falls ein Spotify/media thumbnail haengt.
$readerSource = @'
using System;
using System.IO;
using System.Runtime.InteropServices;
using System.Runtime.InteropServices.ComTypes;
public static class SafeSpotifyCoverReader
{
...
function Score-Source([string]$src, [string]$state, [string]$title, [string]$artist) {
    ...
    if($s.Contains("spotify")){ $score += 500 }
    elseif($s.Contains("chrome")){ $score += 70 }
    elseif($s.Contains("msedge") -or $s.Contains("edge")){ $score += 65 }
    elseif($s.Contains("firefox")){ $score += 60 }
    if($state -eq "Playing"){ $score += 40 }
    if(-not [string]::IsNullOrWhiteSpace($expectedTitle) -and $title -eq $expectedTitle){ $score += 250 }
    if(-not [string]::IsNullOrWhiteSpace($expectedArtist) -and $artist -eq $expectedArtist){ $score += 100 }
    return $score
}
...
try {
    ...
    try {
        [byte[]]$bytes = [SafeSpotifyCoverReader]::ReadAll($ras, 8388608)
    ...
    # V12.8: NIEMALS die media raw data direkt an GTA weiterreichen.
    ...
    $final = Join-Path $coverDir ("spotify_ready_" + $Key + ".png")
    $tmp = Join-Path $coverDir ("spotify_ready_" + $Key + ".png.tmp")
    ...
    try {
        ...
        # TEST67: preserve Spotify artwork and place it on a clean brand-dark card.
        Draw-SpotifyArtworkCard $gfx $img $side
    ...
    try {
        $readyCovers = @(Get-ChildItem -LiteralPath $coverDir -File -Filter "spotify_ready_*.png" -ErrorAction SilentlyContinue |
